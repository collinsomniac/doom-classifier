...
test("real DOOM boots, primes MobileBERT, then keeps the control tick neural-fast",async({page})=>{
  ...
  await expect(page.locator("#stateTable .state-row")).toHaveCount(14);await expect(page.locator("#backboneName")).toContainText("SchemaHashSetNet");

  await page.locator("#stepBtn").click();await expect(page.locator("#steps")).toHaveText("1",{timeout:5000});await expect(page.locator("#eventLog")).toContainText("s0001");
