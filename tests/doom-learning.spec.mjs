...
test("prepared real-Doom policy reports pre/post short fine-tune behavior",async({page})=>{
  ...
  const result=await page.evaluate(async(trainSteps)=>{
    ...
    const evaluate=async(steps)=>{
      ...
      const teacherBefore=p.teacherCalls,counts={},semanticCounts={},valueCounts={};let reward=0,hostileHpLoss=0,attributedDamage=0,playerKills=0,pickups=0,combatAttributionTicks=0,received=0,kills=0,maxP=0,entropy=0,fire=0,switches=0,maxStreak=0,lastAction=null,streak=0,chosenSemantic=0,chosenValue=0,valueBeta=0,priorKL=0,valueTrust=0,klUtilization=0,betaSaturated=0;
