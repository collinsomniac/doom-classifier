...
test("prepared real-Doom policy reports pre/post short fine-tune behavior",async({page})=>{
  ...
  const result=await page.evaluate(async([trainSteps,rolloutHorizon])=>{
    ...
    const evaluate=async(steps)=>{
      ...
      const teacherBefore=p.teacherCalls,counts={},semanticCounts={},valueCounts={};let reward=0,hostileHpLoss=0,attributedDamage=0,playerKills=0,pickups=0,combatAttributionTicks=0,received=0,kills=0,maxP=0,entropy=0,epistemic=0,valueGap=0,priorValueAgree=0,replayEvidence=0,replayDelta=0,replayCriticSupport=0,replayPriorSupport=0,fire=0,switches=0,maxStreak=0,lastAction=null,streak=0,chosenSemantic=0,chosenValue=0,valueBeta=0,priorKL=0,valueTrust=0,klUtilization=0,betaSaturated=0;
      for(let i=0;i<steps;i++){
        ...
        replayEvidence+=Number(d.replayEvidenceConfidence||0);replayDelta+=Number(d.replayEvidenceReturnDelta||0);replayCriticSupport+=Number(d.replayCriticSupport||0);replayPriorSupport+=Number(d.replayPriorSupport||0);if(d.action.id.includes("fire"))fire++;
