...
export class ExperimentController extends EventTarget{
  ...
  schedule(token,delay){
    ...
    this.timer=setTimeout(async()=>{const started=performance.now();await this.tick();const remaining=Math.max(0,1000/this.hz-(performance.now()-started));this.schedule(token,remaining)},delay);
  }
