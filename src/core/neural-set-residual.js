...
function compileSparse(text,dim,baseScale=1){
  const ts=tokens(text),map=new Map();
  if(!ts.length)return[];
  const norm=baseScale/Math.sqrt(ts.length);
  const add=(i,v)=>map.set(i,(map.get(i)||0)+v);
...
function normalizeValue(value,field={}){
  ...
  const scale=Math.max(1e-6,Number(field.scale)||1),x=v/scale;
...
function vectorFromCompiled(compiledFields,record,dim,prefixSparse=null){
  ...
  if(prefixSparse)addSparse(out,prefixSparse,1);
  for(const compiled of compiledFields){
    const {field}=compiled;
    if(record==null||!Object.prototype.hasOwnProperty.call(record,field.id))continue;
    addSparse(out,compiled.valueSparse,normalizeValue(record[field.id],field));
...
function actionVector(action,dim){
  ...
  const out=new Float32Array(dim);addSparse(out,compileSparse(text,dim,1));
...
class Dense{
  ...
  backward(cache,gradOut){
    ...
    for(let o=0;o<this.outputSize;o++){
      let g=gradOut[o];if(this.activation==="tanh")g*=1-cache.out[o]*cache.out[o];
      this.gb[o]+=g;const base=o*this.inputSize;
...
export class NeuralSetResidualQ{
  ...
  encodeState(observation,{cache=false}={}){
    ...
    const recordCaches=cache?[]:null;
    let recordCount=0,collectionCount=0;
    for(const collection of this.compiledCollections){
      const records=observation?._collections?.[collection.id]||[];if(records.length)collectionCount++;
      for(const record of records){
        const raw=vectorFromCompiled(collection.fields,record,this.hashDim,collection.prefixSparse);
        const c1=this.entityLayer1.forward(raw),c2=this.entityLayer2.forward(c1.out),z=c2.out,r=recordCount++;
        for(let d=0;d<this.entityDim;d++){mean[d]+=z[d];if(z[d]>max[d]){max[d]=z[d];maxIndex[d]=r}}
        if(cache)recordCaches.push({c1,c2});
      }
    }
    if(recordCount){for(let d=0;d<this.entityDim;d++)mean[d]/=recordCount}else max.fill(0);
    const context=new Float32Array(this.contextDim);let p=0;
    context.set(globalCache.out,p);p+=this.globalDim;context.set(mean,p);p+=this.entityDim;context.set(max,p);p+=this.entityDim;
    context[p++]=Math.log1p(recordCount)/Math.log(1025);context[p]=Math.log1p(collectionCount)/Math.log(17);
    return{context,cache:cache?{globalCache,recordCaches,maxIndex,recordCount}:null};
  }

  actionForward(context,actionIndex){
    const input=new Float32Array(this.headInputDim);input.set(context,0);input.set(this.actionEmbeddings[actionIndex],this.contextDim);
    const h=this.headLayer.forward(input),o=this.outLayer.forward(h.out);return{score:o.out[0],h,o};
  }
  scoresObservation(observation){
    const state=this.encodeState(observation),scores=new Array(this.actions.length);
    for(let i=0;i<scores.length;i++)scores[i]=this.actionForward(state.context,i).score;return scores;
  }
  backwardState(stateCache,gradContext){
    this.globalLayer.backward(stateCache.globalCache,gradContext.slice(0,this.globalDim));
    const meanGrad=gradContext.slice(this.globalDim,this.globalDim+this.entityDim);
    const maxGrad=gradContext.slice(this.globalDim+this.entityDim,this.globalDim+this.entityDim*2),n=stateCache.recordCount;
    if(!n)return;
    for(let r=0;r<n;r++){
      ...
      const g1=this.entityLayer2.backward(stateCache.recordCaches[r].c2,g);this.entityLayer1.backward(stateCache.recordCaches[r].c1,g1);
    }
  }
  backwardAction(forward,gradScore){
    const gh=this.outLayer.backward(forward.o,new Float32Array([gradScore]));return this.headLayer.backward(forward.h,gh);
  }
  updateTransition({observation,actionIndex,reward,nextObservation,done=false}){
    const current=this.encodeState(observation,{cache:true}),chosen=this.actionForward(current.context,actionIndex);
    const nextScores=done?[]:this.scoresObservation(nextObservation),target=reward+(done?0:this.gamma*Math.max(...nextScores)),td=clamp(target-chosen.score,-4,4);
    this.zeroGrad();const gradInput=this.backwardAction(chosen,-td);this.backwardState(current.cache,gradInput.slice(0,this.contextDim));this.apply();this.updates++;
  ...
  distill(observation,teacherScores,{strength=.35}={}){
    ...
    const state=this.encodeState(observation,{cache:true}),forwards=this.actions.map((_,i)=>this.actionForward(state.context,i));
    const student=softmax(forwards.map(x=>x.score),1),teacher=softmax(teacherScores,1),gradContext=new Float32Array(this.contextDim);
    this.zeroGrad();let loss=0;
    for(let i=0;i<this.actions.length;i++){
      loss-=teacher[i]*Math.log(Math.max(1e-8,student[i]));
      const grad=this.backwardAction(forwards[i],student[i]-teacher[i]);for(let j=0;j<this.contextDim;j++)gradContext[j]+=grad[j];
    }
    this.backwardState(state.cache,gradContext);this.apply(this.lr*strength);this.distillUpdates++;return{loss,student,teacher};
  }
}
