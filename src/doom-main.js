...
function renderArchitecture(obs,d,outcome){
  ...
  setArchNode("value",{active:(policy?.q?.updates||0)>0||!!d,hot:learning,learning,value:valueTop>=0?("value → "+policy.actions[valueTop].id+" · ΔQ "+valueGap.toFixed(4)+" · heads "+(Number(d?.criticTopAgreement||0)*100).toFixed(0)+"% · SNR "+Number(d?.criticMarginSnr||0).toFixed(1)+" · "+(policy.q.updates||0)+" updates"):"untrained"});
  setArchNode("fusion",{active:!!d,hot:!!d&&Number(d.valueBeta||0)>0,value:d?("β "+Number(d.valueBeta||0).toFixed(2)+" · KL "+(Number(d.klUtilization||0)*100).toFixed(0)+"% · EPI "+(Number(d.epistemicUtilization||0)*100).toFixed(0)+"% · → "+d.action.id):"β 0 · KL 0% · EPI 0%"});
