...
function renderArchitecture(obs,d,outcome){
  ...
  setArchNode("fusion",{active:!!d,hot:!!d&&Number(d.valueBeta||0)>0,value:d?("β "+Number(d.valueBeta||0).toFixed(2)+" · critic "+(Number(d.criticRankingConfidence||0)*100).toFixed(0)+"% · replay "+(Number(d.replayEvidenceConfidence||0)*100).toFixed(0)+"% ΔR "+Number(d.replayEvidenceReturnDelta||0).toFixed(3)+" · support "+Number(d.replayCriticSupport||0).toFixed(1)+"/"+Number(d.replayPriorSupport||0).toFixed(1)+" · KL "+Number(d.priorKlBudget||0).toFixed(3)+" · → "+d.action.id):"β 0 · critic/replay evidence waiting"});
