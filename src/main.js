...
const policy=new SemanticResidualPolicy({schema:env.schema,actions:env.actions,semantic});
const controller=new ExperimentController({environment:env,policy,hz:20});

const $=id=>document.getElementById(id);
const ui={
  start:$("startBtn"),step:$("stepBtn"),reset:$("resetBtn"),learning:$("learningToggle"),memory:$("memoryToggle"),explore:$("exploreToggle"),
  rate:$("rateInput"),rateOut:$("rateOut"),fsm:$("fsmState"),dot:$("statusDot"),bars:$("actionBars"),chosen:$("chosenAction"),
  entropy:$("entropy"),margin:$("margin"),novelty:$("novelty"),latLast:$("latLast"),latP50:$("latP50"),latP95:$("latP95"),latP99:$("latP99"),
  steps:$("steps"),episodes:$("episodes"),ret:$("return"),updates:$("updates"),state:$("stateTable"),log:$("eventLog"),export:$("exportBtn"),
  arena:$("arena"),chart:$("rewardChart")
};
...
function buildBars(){
  ...
  for(const a of env.actions){
    ...
    row.append(label,track,val);ui.bars.append(row);
  }
}
buildBars();

function renderArena(){
  const c=ui.arena,ctx=c.getContext("2d"),w=c.width,h=c.height;
  ctx.clearRect(0,0,w,h);ctx.fillStyle="#090d12";ctx.fillRect(0,0,w,h);ctx.strokeStyle="#1d2630";ctx.lineWidth=1;
...
function renderChart(){
  const c=ui.chart,ctx=c.getContext("2d"),xs=controller.returns.slice(-60);
  ctx.clearRect(0,0,c.width,c.height);ctx.strokeStyle="#303946";ctx.beginPath();ctx.moveTo(0,c.height/2);ctx.lineTo(c.width,c.height/2);ctx.stroke();
  if(xs.length<2)return;const min=Math.min(-1,...xs),max=Math.max(1,...xs);ctx.strokeStyle="#d9ff5a";ctx.lineWidth=2;ctx.beginPath();
  xs.forEach((v,i)=>{const x=i/(xs.length-1)*c.width,y=c.height-(v-min)/(max-min)*c.height;i?ctx.lineTo(x,y):ctx.moveTo(x,y)});ctx.stroke();
}
...
function render(){
  ...
  const lat=controller.latencySummary();ui.latLast.textContent=lat.last.toFixed(2)+" ms";ui.latP50.textContent=lat.p50.toFixed(2)+" ms";ui.latP95.textContent=lat.p95.toFixed(2)+" ms";ui.latP99.textContent=lat.p99.toFixed(2)+" ms";
  const d=controller.lastDecision;
  if(d){
    ui.chosen.textContent=d.action.label;ui.entropy.textContent=d.uncertainty.entropy.toFixed(3);ui.margin.textContent=d.uncertainty.margin.toFixed(3);ui.novelty.textContent=d.uncertainty.novelty.toFixed(3);
...
ui.step.addEventListener("click",()=>{if(controller.state==="RUNNING")controller.pause();controller.tick()});
...
ui.learning.addEventListener("change",()=>controller.learning=ui.learning.checked);
ui.memory.addEventListener("change",()=>controller.memory=ui.memory.checked);
ui.explore.addEventListener("change",()=>controller.explore=ui.explore.checked);
ui.rate.addEventListener("input",()=>{ui.rateOut.value=ui.rate.value;controller.setHz(Number(ui.rate.value))});
ui.export.addEventListener("click",()=>{
  const blob=new Blob([controller.exportTrace()],{type:"application/json"}),a=document.createElement("a");
  a.href=URL.createObjectURL(blob);a.download="doom-classifier-trace-"+Date.now()+".json";a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000);
});
render();
